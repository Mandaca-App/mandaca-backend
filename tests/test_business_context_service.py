...
# ---------------------------------------------------------------------------
# create  (dados_contexto chega como str — service chama _parse_dados)
# ---------------------------------------------------------------------------


def test_given_valid_json_string_when_create_then_persists_parsed_dict():
    # GIVEN
    db = _mock_db()
    db.get.return_value = _make_enterprise()
    payload = BusinessContextCreate(
        dados_contexto=FAKE_DADOS_STR,
        empresa_id=FAKE_ENTERPRISE_ID,
    )

    # WHEN
    business_context_service.create(payload, db)
    ...
    assert added.dados_contexto["nome"] == "Restaurante Teste"


def test_given_valid_json_string_when_create_then_computes_hash():
    # GIVEN
    db = _mock_db()
    db.get.return_value = _make_enterprise()
    payload = BusinessContextCreate(
    ...
    assert added.hash_contexto != ""


def test_given_invalid_json_string_when_create_then_raises_invalid_context_data():
    # GIVEN
    db = _mock_db()
    db.get.return_value = _make_enterprise()
    payload = BusinessContextCreate(
        dados_contexto="isso nao e json valido {{{",
        empresa_id=FAKE_ENTERPRISE_ID,
    )

    # WHEN / THEN
    with pytest.raises(InvalidContextDataError):
        business_context_service.create(payload, db)


def test_given_invalid_json_when_create_then_does_not_persist():
    # GIVEN
    db = _mock_db()
    db.get.return_value = _make_enterprise()
    payload = BusinessContextCreate(
        dados_contexto="nao-e-json",
        empresa_id=FAKE_ENTERPRISE_ID,
    )
...
def test_given_missing_enterprise_when_create_then_raises_not_found():
    # GIVEN
    db = _mock_db()
    db.get.return_value = None
    payload = BusinessContextCreate(
